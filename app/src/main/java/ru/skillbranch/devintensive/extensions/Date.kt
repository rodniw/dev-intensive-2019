...
import ru.skillbranch.devintensive.extensions.Plurals.MANY
import ru.skillbranch.devintensive.extensions.Plurals.ONE
...
val TimeUnits.pluralStrings
    get() = when (this) {
        TimeUnits.SECOND -> mapOf(ONE to "секунду", Plurals.FEW to "секунды", MANY to "секунд")
        TimeUnits.MINUTE -> mapOf(ONE to "минуту", Plurals.FEW to "минуты", MANY to "минут")
        TimeUnits.HOUR -> mapOf(ONE to "час", Plurals.FEW to "часа", MANY to "часов")
        TimeUnits.DAY -> mapOf(ONE to "день", Plurals.FEW to "дня", MANY to "дней")
    }
